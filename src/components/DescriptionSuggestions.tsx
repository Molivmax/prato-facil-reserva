...
const DescriptionSuggestions: React.FC<DescriptionSuggestionsProps> = ({ onSelect }) => {
  ...
  const generateDescriptions = () => {
    if (!type || !specialties) return [];

    const shortDesc = `${type} especializado em ${specialties}. ${atmosphere}`;
    
    const mediumDesc = `${type} acolhedor com ${specialties}. Oferecemos uma experiência única com ${atmosphere.toLowerCase()}`;
    
    const longDesc = `Bem-vindo ao nosso ${type.toLowerCase()} onde nos especializamos em ${specialties}. ${atmosphere} Venha nos conhecer e desfrute de uma experiência gastronômica única.`;
